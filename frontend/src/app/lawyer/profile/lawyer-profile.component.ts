// src/app/lawyer/profile/lawyer-profile.component.ts
...
export class LawyerProfileComponent implements OnInit {
  ...
  loadProfile(): void {
    this.profileService.getLawyerProfile().subscribe({
      ...
      error: err => { this.error = err?.error?.message ?? 'Failed to load profile.'; this.loading = false; }
    });
  }
  ...
  saveProfile(): void {
    ...
    this.saving    = true;
    this.saveError = '';
    this.profileService.updateLawyerProfile(this.editForm).subscribe({
      next: updated => {
  ...
  /** Only show hints once the user starts typing */
  ...
  submitPasswordChange(): void {
    ...
    this.profileService.changeLawyerPassword(this.pwForm).subscribe({
      next: () => {
        this.pwSaving  = false;
